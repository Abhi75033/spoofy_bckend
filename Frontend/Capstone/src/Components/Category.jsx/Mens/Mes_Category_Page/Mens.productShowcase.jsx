  ...
  const getPro = async () => {
    try {
      const resp = await axios.get(
        "http://localhost:3000/api/v1/product/getlimiPro2"
      );
      const respdata = resp.data.data.filter((item)=>item.Gender=='Male')
...
export default MensProductShowcase;
